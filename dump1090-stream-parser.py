...
def main():
    ...
    while count_failed_connection_attempts < args.connect_attempt_limit:
        try:
            ...
            count_failed_connection_attempts = 1
            log.debug("Connected to dump1090 broadcast")
            break
        except socket.error:
            count_failed_connection_attempts += 1
            log.warn(f"Cannot connect to dump1090 broadcast. Making attempt {count_failed_connection_attempts}")
    ...
    try:
        ...
        while True:
            ...
            if len(message) == 0:
                log.warn(ts, "No broadcast received. Attempting to reconnect")
                time.sleep(args.connect_attempt_delay)
                s.close()
                ...
                while count_failed_connection_attempts < args.connect_attempt_limit:
                    try:
                        ...
                        count_failed_connection_attempts += 1
                        log.error(f"The attempt failed. Making attempt {count_failed_connection_attempts}")
                        time.sleep(args.connect_attempt_delay)
                else:
                    quit()
            ...
            for d in data:
                ...
                if len(line) == 22:
                    ...
                    lat, lon, el, gs = (line[14], line[15], line[11], line[12])
                    if lat and lon and el:

                        log.debug(line)
                        try:
                            az, el, d = azel((float(lat), float(lon), float(el)), verbose=False)
                            print(az, el, d)
                            line.append(az)
                            line.append(el)

                        except:
                            log.warn(f'failed getting az,el for lat={lat} lon={lon} el={el}')
                            line.append('')
                            line.append('')
                    ...
                    try:
                        ...
                        if count_since_commit % args.batch_size == 0:
                            ...
                            if cur_time != last_time:
                                log.debug("averging {0} rows per second, currently {1} rows per second".format(float(count_total) / (cur_time - start_time).total_seconds(), float(count_since_commit) / (cur_time - last_time).total_seconds()))
                            else:
                                log.debug("averging {0} rows per second".format(float(count_total) / (cur_time - start_time).total_seconds(),))
                            if count_since_commit > args.batch_size:
                                log.debug(ts, "All caught up, {0} rows, successfully written to database".format(count_since_commit))
                            count_since_commit = 0
                            last_time = cur_time

                    except sqlite3.OperationalError:
                        log.warn(ts, "Could not write to database, will try to insert {0} rows on next commit".format(count_since_commit + args.batch_size,))
